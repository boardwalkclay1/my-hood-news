...
function renderPage2(city) {
  ...
  container.innerHTML = `
    <div class="column">
      ...
      ${renderList(local.articles)}
    </div>
    <div class="column">
      <h2>Crime</h2>
      ${renderImage(crime.image)}
      ${renderList(crime.articles)}
    </div>
  `;
}
...
function renderPage3(city) {
  ...
  container.innerHTML = `
    <div class="column">
      ...
      ${renderList(events.articles)}
    </div>
    <div class="column">
      <h2>Community Help</h2>
      ${renderImage(help.image)}
      ${renderList(help.articles)}
    </div>
  `;
}
...
function renderPage4(city) {
  ...
  container.innerHTML = `
    <div class="column">
      ...
      ${renderList(business.articles)}
    </div>
    <div class="column">
      <h2>Sports</h2>
      ${renderImage(sports.image)}
      ${renderList(sports.articles)}
    </div>
  `;
}
...
function renderPage5(city) {
  ...
  container.innerHTML = `
    <div class="column">
      ...
      ${renderList(opinion.articles)}
    </div>
    <div class="column">
      <h2>Feature</h2>
      ${renderImage(feature.image)}
      ${renderList(feature.articles)}
    </div>
  `;
}
  ...
  container.innerHTML = `
    ...
    <div class="column">
      ...
      <div class="extra-section legal-section">
        ...
        <ul>${legal.sources.map(s => `<li>${s}</li>`).join("")}</ul>
      </div>
    </div>
  `;
}
...
function renderImage(url) {
  return url ? `<img src="${url}" class="article-image" />` : "";
}

function renderLink(url) {
  return url ? `<p><a href="${url}" target="_blank">Read more</a></p>` : "";
}
